...
class GetChain():
    ...
    def get(self, campaign):
        ...
        while True:
            ...
            for arg in arg_list:
                if not arg.startswith('chain'):
                    ...
                    ccs = []
                    for c in self.es.search(('campaigns:%s' % arg), index='chained_campaigns', size=self.overflow)['hits']['hits']:
        ...
        for a_cc in arg_list:
            try:
                mcm_cc = self.es.get('chained_campaigns', 'chain_campaign', a_cc)['_source']
            except Exception:
                # try to see if that's a flow
                return "%s does not exists" % ( a_cc )
            all_cc[a_cc] = mcm_cc ## keep it in mind
            all_cr.extend([s['_source'] for s in self.es.search(('member_of_campaign:%s' % a_cc), index='chained_requests', size=self.overflow)['hits']['hits']])
        ...
        all_requests = {}
        for step in steps:
            for r in [s['_source'] for s in self.es.search(('member_of_campaign:%s' % step), index='requests', size=self.overflow)['hits']['hits']]:
...
class GetCampaign():
    ...
    def get(self, campaign):
        ...
        if campaign == 'all':
            campaign = '*'
        for s in self.es.search(('member_of_campaign:%s' % campaign),
                                index='requests', size=self.overflow)['hits']['hits']:
